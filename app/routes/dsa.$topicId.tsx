...
// Loader fetches the specific DSA topic
export async function loader({ params }: LoaderFunctionArgs) {
  invariant(params.topicId, "Missing topicId param");
  const topic = getTopic(params.topicId, 'dsa'); // Ensure we only load DSA topics here
  if (!topic) {
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  return [
    { title: `${data?.title || 'DSA Topic'} | JavaLearn` },
    { name: "description", content: `Learn about ${data?.title || 'this DSA topic'}.` },
  ];
};

// Component to display a single code example with copy button
// (Re-using the component structure from topics.$topicId.tsx)
...
// Main component to display the DSA topic page
...
// Route-specific Error Boundary for DSA topics
export function ErrorBoundary() {
  const params = useParams();
  return (
    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
      <strong className="font-bold">Error!</strong>
      <span className="block sm:inline"> Could not load the DSA topic "{params.topicId}". It might not exist.</span>
      <p><Link to="/dsa" className="text-blue-600 hover:underline">Return to DSA Topics</Link></p>
    </div>
  );
}
